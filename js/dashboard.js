...
const DashboardModule = (() => {
  ...
  async function render(container, tickers, settings, onSelect) {
    ...
    if (!validCards.length) {
      container.innerHTML = '<div class="loading">داده‌ای برای نمایش نیست. لطفاً از دکمه‌ی «اتصال پوشه sahm» استفاده کنید (هر نماد حداقل به ۳۰ کندل نیاز دارد).</div>';
      return;
    }
    grid.innerHTML = validCards.map(c => c.html).join('');
